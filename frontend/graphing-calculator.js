// graphing-calculator.js - Calculadora gráfica (canvas, sin IA)
...
function gcRender() {
  ...
  gcFunctions.forEach((fn, i) => {
    ...
    <div style="display:flex;gap:0.4rem;align-items:center;margin-bottom:0.5rem">
      <input type="color" value="${fn.color}" onchange="gcSetColor(${i},this.value)" style="width:28px;height:28px;border:none;cursor:pointer;border-radius:4px">
      <span style="color:var(--text-muted);font-size:0.85rem">f(x) =</span>
      <input value="${fn.expr}" oninput="gcSetExpr(${i},this.value)" placeholder="ej: x^2, sin(x), 2x+1" style="flex:1;background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:6px;padding:0.4rem 0.6rem;font-size:0.85rem;color:var(--text-primary);font-family:'Cambria Math',monospace;outline:none">
      ${gcFunctions.length > 1 ? `<button onclick="gcRemoveFn(${i})" style="background:none;border:none;cursor:pointer;color:var(--text-muted);font-size:1rem">×</button>` : ''}
    </div>`;
  });

  html += `<button onclick="gcAddFn()" style="background:var(--bg-secondary);border:1px dashed var(--border-color);border-radius:6px;padding:0.4rem 0.8rem;cursor:pointer;font-size:0.8rem;color:var(--text-muted);margin-bottom:0.8rem;width:100%">+ Agregar función</button>`;

  html += `<canvas id="gc-canvas" style="width:100%;height:400px;border:1px solid var(--border-color);border-radius:8px;cursor:crosshair;background:var(--bg-card)"></canvas>`;

  html += `<div id="gc-tooltip" style="display:none;position:fixed;background:var(--bg-card);border:1px solid var(--border-color);border-radius:6px;padding:0.3rem 0.6rem;font-size:0.78rem;color:var(--text-primary);pointer-events:none;z-index:100;box-shadow:0 2px 8px rgba(0,0,0,0.15)"></div>`;
...
function gcZoom(factor) { gcScale = Math.max(5, Math.min(200, gcScale * factor)); gcDraw(); }
function gcReset() { gcScale = 40; gcOffsetX = 0; gcOffsetY = 0; gcDraw(); }
function gcAddFn() { gcFunctions.push({ expr: '', color: GC_COLORS[gcFunctions.length % GC_COLORS.length] }); gcRender(); }
function gcRemoveFn(i) { gcFunctions.splice(i, 1); gcRender(); }
function gcSetExpr(i, v) { gcFunctions[i].expr = v; gcDraw(); }
function gcSetColor(i, v) { gcFunctions[i].color = v; gcDraw(); }

window.gcRender = gcRender;
window.gcZoom = gcZoom;
window.gcReset = gcReset;
window.gcAddFn = gcAddFn;
window.gcRemoveFn = gcRemoveFn;
window.gcSetExpr = gcSetExpr;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    const cx = rect.width / 2 + gcOffsetX, cy = rect.height / 2 + gcOffsetY;
    const unitsPerPx = 1 / gcScale;
    const x = (mx - cx) * unitsPerPx;
  ...
  canvas.addEventListener('touchstart', e => {
    if (e.touches.length === 1) {
      gcDragging = true;
      gcDragStart = { x: e.touches[0].clientX - gcOffsetX, y: e.touches[0].clientY - gcOffsetY };
    }
  }, { passive: true });
