// biblioteca-tiza.js — CBC UBA: contenido de Altillo (altillo.com)

const BIBLIC_TIZA = { currentView: "home", currentSubject: null, currentUnit: null };

// ═══════════════════════════════════════════════════════
// MATERIAS CBC INGENIERÍA (contenido de Altillo)
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// PARCIALES (información de Altillo)
// ═══════════════════════════════════════════════════════
...
function renderHome(el) {
  ...
  el.innerHTML = `<div style="max-width:820px;margin:0 auto;padding:1.5rem 1.2rem">
    <div style="text-align:center;margin-bottom:1.5rem">
      ...
      <p style="color:var(--text-muted);font-size:0.65rem;margin-top:0.2rem">Contenido de <a href="https://altillo.com/fiuba" target="_blank" style="color:#8b5cf6;text-decoration:none">altillo.com</a></p>
    </div>
    <div style="background:linear-gradient(135deg,rgba(139,92,246,0.08),rgba(59,130,246,0.05));border:1px solid rgba(139,92,246,0.2);border-radius:12px;padding:0.8rem 1rem;margin-bottom:1rem">
      <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.5rem"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg><span style="font-size:0.78rem;font-weight:700;color:var(--text-primary)">Plan de estudio</span></div>
...
function renderUnit(el) {
  ...
  el.innerHTML = `<div style="max-width:820px;margin:0 auto;padding:1.5rem 1.2rem">
    ...
    <div style="background:linear-gradient(135deg,rgba(239,68,68,0.06),rgba(220,38,38,0.03));border:1px solid rgba(239,68,68,0.2);border-radius:12px;padding:0.7rem;margin-bottom:0.8rem">
      <div style="font-size:0.75rem;font-weight:700;color:#ef4444;margin-bottom:0.3rem;display:flex;align-items:center;gap:0.4rem"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> Donde se cae la mayoría</div>
      <div style="font-size:0.68rem;color:var(--text-muted);line-height:1.5">Cada tema del parcial tiene errores típicos que comete la mayoría. Consultá las guías y parciales resueltos en <a href="https://altillo.com/fiuba" target="_blank" style="color:#8b5cf6;text-decoration:none">altillo.com</a>.</div>
    </div>
    ...
    <div style="margin-top:1rem;padding:0.7rem;background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px;text-align:center">
      <a href="https://altillo.com/fiuba" target="_blank" style="font-size:0.75rem;color:${mat.color};text-decoration:none;font-weight:600">→ Ver guías y parciales en Altillo</a>
...
function renderParcial(el) {
  ...
  el.innerHTML = `<div style="max-width:820px;margin:0 auto;padding:1.5rem 1.2rem">
    <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.6rem">
      <button onclick="BIBLIC_TIZA.currentView='subject';renderBiblioteca()" style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;padding:0.25rem 0.5rem;cursor:pointer;font-size:0.68rem;color:var(--text-muted)">← ${mat.name}</button>
      <h2 style="font-size:0.95rem;font-weight:700;color:var(--text-primary);margin:0;display:flex;align-items:center;gap:0.4rem"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> ${ex.nombre}</h2>
    </div>
    ...
    <div style="margin-bottom:0.8rem"><div style="font-size:0.75rem;font-weight:700;color:var(--text-primary);margin-bottom:0.3rem;display:flex;align-items:center;gap:0.4rem"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg> Temas del parcial</div><div style="display:flex;flex-wrap:wrap;gap:0.25rem">${ex.temas.map(t=>`<span style="font-size:0.58rem;padding:0.12rem 0.4rem;background:${mat.color}15;border:1px solid ${mat.color}30;border-radius:5px;color:${mat.color};font-weight:600">${t}</span>`).join("")}</div></div>
    <div style="background:linear-gradient(135deg,rgba(139,92,246,0.08),rgba(59,130,246,0.05));border:1px solid rgba(139,92,246,0.2);border-radius:12px;padding:0.8rem;text-align:center;margin-bottom:0.8rem">
      <div style="font-size:0.78rem;font-weight:700;color:var(--text-primary);margin-bottom:0.3rem;display:flex;align-items:center;gap:0.4rem"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> Parcial resuelto paso a paso</div>
      <div style="font-size:0.68rem;color:var(--text-muted);margin-bottom:0.5rem">Los parciales modelo están escritos con el temario y formato reales de FIUBA.</div>
      <a href="https://altillo.com/fiuba/cbc/${mat.id === 'am1' ? 'analisis-matematico-i' : mat.id === 'algebra' ? 'algebra-y-geometria-analitica' : mat.id === 'fisica' ? 'fisica-i' : mat.id === 'penscomp' ? 'pensamiento-computacional' : mat.id === 'ipc' ? 'introduccion-al-pensamiento-cientifico' : 'sociedad-y-estado'}/parciales" target="_blank" style="display:inline-block;padding:0.4rem 1rem;background:${mat.color};color:white;border-radius:8px;font-size:0.7rem;font-weight:600;text-decoration:none">Ver parciales en Altillo →</a>
    </div>
    <div style="padding:0.6rem;background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px">
      <div style="font-size:0.7rem;font-weight:700;color:var(--text-primary);margin-bottom:0.2rem;display:flex;align-items:center;gap:0.4rem"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 1 1 7.072 0l-.548.547A3.374 3.374 0 0 0 14 18.469V19a2 2 0 1 1-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg> Antes de rendir</div>
...
function renderPlan(el) {
  ...
  el.innerHTML = `<div style="max-width:820px;margin:0 auto;padding:1.5rem 1.2rem">
    <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem">
      <button onclick="BIBLIC_TIZA.currentView='subject';renderBiblioteca()" style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;padding:0.25rem 0.5rem;cursor:pointer;font-size:0.68rem;color:var(--text-muted)">← ${mat.name}</button>
      <h2 style="font-size:0.95rem;font-weight:700;color:var(--text-primary);margin:0;display:flex;align-items:center;gap:0.4rem"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg> Plan de estudio</h2>
    </div>
    ...
    <div style="margin-top:0.8rem;padding:0.7rem;background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px">
      <div style="font-size:0.72rem;font-weight:700;color:var(--text-primary);margin-bottom:0.3rem;display:flex;align-items:center;gap:0.4rem"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 1 1 7.072 0l-.548.547A3.374 3.374 0 0 0 14 18.469V19a2 2 0 1 1-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg> Tips</div>
