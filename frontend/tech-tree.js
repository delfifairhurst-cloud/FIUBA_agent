...
function khStyles() {
  ...
  s.textContent = `
    ...
    .kh-btn-p { background:var(--accent); color:white; border:none; font-weight:600; }
  ...
  el.innerHTML = `
    <div class="kh-bar" style="justify-content:space-between">
      ...
      <div style="display:flex;gap:0.3rem">
        ...
        <button class="kh-btn kh-btn-p" onclick="khNew()">+ Nota</button>
      </div>
    </div>
    <div class="kh-bar">
...
function khCanvas() {
  ...
  const cc={"mecánica":"#8b5cf6","termodinámica":"#ef4444","matemática":"#3b82f6","computación":"#22c55e","ingeniería":"#f59e0b","ia":"#ec4899","default":"#6b7280"};
  ...
  function draw(){
    ...
    edges.forEach(e=>{
      ...
      if(isH){const pt=(t*2)%1,ppx=s.x+(tt.x-s.x)*pt,ppy=s.y+(tt.y-s.y)*pt-Math.sin(pt*Math.PI)*20;ctx.beginPath();ctx.arc(ppx,ppy,3,0,Math.PI*2);ctx.fillStyle=c;ctx.fill();}
    });
  ...
  c.onmouseup=()=>{if(drag)khOpen(drag.id);drag=null;pan=false;lm=null;c.style.cursor=KH.hover?"pointer":"grab";};
    ...
    <div style="display:flex;gap:0.8rem;flex-wrap:wrap">
      <div style="flex:2;min-width:300px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:0.5rem;flex-wrap:wrap;gap:0.3rem">
          ...
          <button class="kh-btn kh-btn-p" onclick="khSave('${n.id}')" style="padding:0.4rem 1.2rem">Guardar</button>
        </div>
      </div>
