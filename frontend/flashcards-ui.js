...
async function refreshFlashcardsView() {
  ...
  grid.innerHTML = all.map(function(c) {
    return '<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:12px;padding:0.9rem;display:flex;flex-direction:column;gap:0.5rem">' +
...
window.handleDeleteFlashcard = async function(id) {
  if (!confirm("¿Eliminar flashcard?")) return;
  await deleteFlashcard(id);
  refreshFlashcardsView();
};

window.openFlashcardsGenerateModal = function() {
  if (!getUid()) { alert("Iniciá sesión"); return; }
  document.getElementById("flashcards-generate-modal").classList.add("open");
};
window.closeFlashcardsGenerateModal = function() {
  document.getElementById("flashcards-generate-modal").classList.remove("open");
};

window.handleFlashcardsFileUpload = function(e) {
  var file = e.target.files[0];
  if (!file) return;
  var label = document.getElementById("flashcards-file-label");
  if (label) label.textContent = "📄 " + file.name;
};

window.handleGenerateFlashcards = async function() {
  var fileInput = document.getElementById("flashcards-file-input");
  var select = document.getElementById("flashcards-doc-select");
  var count = document.getElementById("flashcards-count-select") ? document.getElementById("flashcards-count-select").value : "8";
  var rawText = "";

  // Opción 1: subió archivo directamente
  if (fileInput && fileInput.files && fileInput.files.length > 0) {
    var btn = document.getElementById("btn-generate-flashcards");
  ...
  // Opción 2: eligió de Materiales
  else if (select && select.value !== "") {
    var chat = window.getActiveChat ? window.getActiveChat() : null;
    var docs = (chat && chat.loadedDocuments) ? chat.loadedDocuments : [];
    var doc = docs[parseInt(select.value)];
    if (!doc || !doc.text || doc.text.trim().length < 40) {
      alert("Elegí un documento válido con texto");
      return;
    }
    rawText = doc.text;
  } else {
    alert("Subí un PDF o elegí uno de Materiales");
    return;
  }
  ...
  try {
    var apiBase = (window.getApiBase ? window.getApiBase() : (localStorage.getItem('fiuba_agent_api_base') || 'https://fiuba-agent-backend-1.onrender.com')).replace(/\/+$/,'');
    var userApiKey = (window.getUserGeminiKey ? window.getUserGeminiKey() : (localStorage.getItem('fiuba_gemini_key')||''));
    var resp = await fetch(apiBase + '/api/generate-flashcards', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ rawText: rawText, count: parseInt(count), userApiKey: userApiKey })
    });
...
window.startFlashcardsQuiz = async function() {
  var due = await getDueFlashcards();
  var all = await getFlashcards();
...
function renderFlashcardQuiz() {
  ...
  var c = currentQueue[currentIdx];
  var cardEl = document.getElementById("flashcards-quiz-card");
  var backHtml = showingBack
    ? '<div style="background:rgba(255,255,255,0.06);border:1px solid var(--border-color);border-radius:10px;padding:0.8rem;font-size:0.9rem;color:var(--text-secondary)">' + c.back + '</div>'
    : '<button class="btn-secondary" onclick="revealFlashcard()" style="width:100%;justify-content:center">Mostrar respuesta</button>';
  cardEl.innerHTML = '<div style="font-size:0.7rem;color:var(--accent-cyan);font-weight:600;margin-bottom:0.4rem">' + c.topic + ' · ' + (currentIdx+1) + '/' + currentQueue.length + '</div>' +
    '<div style="font-size:1.05rem;font-weight:700;color:var(--text-primary);margin-bottom:0.8rem">' + c.front + '</div>' + backHtml;

  var actions = document.getElementById("flashcards-quiz-actions");
  if (showingBack) {
    actions.innerHTML = '<button onclick="rateFlashcard(0)" style="flex:1;background:rgba(239,68,68,0.15);color:var(--danger);border:1px solid var(--danger);border-radius:8px;padding:0.5rem;font-size:0.8rem;cursor:pointer">Otra vez</button>' +
...
window.revealFlashcard = function() { showingBack = true; renderFlashcardQuiz(); };
window.rateFlashcard = async function(q) {
  var c = currentQueue[currentIdx];
  try { await reviewFlashcard(c.id, q); } catch(e) { console.error(e); }
  if (window.Gamification) {
    window.Gamification.trackFlashcard(q >= 3);
    var r = window.Gamification.addXp(q >= 3 ? 'flashcard_mastered' : 'flashcard_reviewed');
...
window.closeFlashcardsQuiz = function() { document.getElementById("flashcards-quiz-modal").classList.remove("open"); };

authReady.then(function() { refreshFlashcardsView(); });
